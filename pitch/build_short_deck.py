"""
Build the 3-slide Inibsa Alert Marker pitch deck (~2 min talk + 3 min demo).

Design:
  - Light, minimal — white background
  - Mythos brand: navy + steel-blue accents
  - Slide 1: cover (recreates the Mythos / Inibsa Alert Marker layout)
  - Slide 2: opportunity + approach (combined)
  - Slide 3: numbers + brief scorecard + thanks

If pitch/cover.png exists, slide 1 uses the actual image. Otherwise we recreate
the design programmatically.
"""
...
# White background (default — leave blank)

cover_path = ROOT / "pitch" / "cover.png"
if cover_path.exists():
    s.shapes.add_picture(str(cover_path), 0, 0, width=Inches(13.333), height=Inches(7.5))
else:
    # Recreate the layout programmatically.
    # Left half: stylized "M" logo with bar-chart bars + "MYTHOS" wordmark
    logo_x = Inches(2.0)
    logo_y = Inches(2.0)
    logo_size = Inches(2.6)

    # Build "M" using two thick navy bars + bar-chart bars in the middle
    # Left vertical of M
    left_v = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, logo_x, logo_y,
                                Inches(0.40), logo_size)
    fill(left_v, NAVY)
    # Right vertical of M
    right_v = s.shapes.add_shape(MSO_SHAPE.RECTANGLE,
                                  ...
                                  Inches(0.40), logo_size)
    fill(right_v, NAVY)
    # Top connecting strokes (two diagonals approximated as thin bars)
    ...
    # Stair-step bar chart inside the M
    ...
    # MYTHOS wordmark + thin underline
...
# Two cards: Commodities (light) + Technical (navy)
...
# Three big metrics (stacked horizontally)
metrics = [
...
# Brief scorecard — 8 deliverables in two rows
deliverables = [
...
# ---- Save ----
out = ROOT / "pitch" / "Inibsa_Alert_Marker_short.pptx"
prs.save(out)
